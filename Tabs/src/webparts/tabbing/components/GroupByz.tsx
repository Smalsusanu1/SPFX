...
import { FaAngleDown, FaAngleUp, FaPrint, FaFileExcel, FaPaintBrush, FaEdit, FaSearch, FaExpandAlt } from 'react-icons/fa';
import { MdDelete, MdAdd, MdOutlineCompareArrows } from 'react-icons/Md';
import { CSVLink } from "react-csv";
import * as React from "react";
import AddStructure from "./AddStructure/Addstructure";
...
//  Testing for global search

const [searchText, setSearchText] : [string, (search: string) => void] = React.useState("");
const [datas, setDatas] = useState([]);
const [searchs, setSearchs]: [string, (search: string) => void] = React.useState("");
const [searchcs, setSearchcs]: [string, (search: string) => void] = React.useState("");
const excludeColumns = ["id", "color"];

// test new way

  const handleChangesn = (e: { target: { value: string; }; }) => {
...
// Testing End Here
    ...
    let handleChange = (e: { target: { value: string; }; }) => {
        ...
        setSearchs(e.target.value.toLowerCase());
    };
        ...
        if (lowercasedValue === "") setDatas(data);
          ...
          const filteredData = data.filter(item => {
            return Object.keys(item.Title).some(key =>
              excludeColumns.indexOf(key) ? false : item[key].toString().toLowerCase().includes(lowercasedValue)
            );
          });
                                ...
                                <div className="col-sm-12 tab-content">
                                    <div className="col-md-5">
                                        <div className="row">
                                            ...
                                            <div className="col-sm-4 mb-10 Doc-align padR0">
                                                <label className="full_width">Item Type
                                                </label>
                                                <select className="form-control" value="2">
                                                    <option value="">Component Category</option>
...
<SmartFilter/>
          ...
          <div className="row tbl-headings">
                <span className="leftsec w47">
                    {/* <button type="button"
                        className="btn btn-primary pull-left" onClick={addModal}>
                        Add Item
                    </button> */}
                    <label>
                    ...
                    <span className="g-search tdcolm_relative">
                        <input type="text" className="searchbox_height full_width" id="globalSearch" placeholder="search all" onChange={handleChange}/>
                                        ...
                                        <tr className="table table-striped">
                                            ...
                                            <th style={{ width: "20%" }}>
                                                <div style={{ width: "100%" }} className="tdcolm-relative">
                                                    <input type="search" placeholder="Title" className="full_width" onChange={handleChangesn} />

                                                   <div style={{position: "absolute",top: "-9px",left: "167px"}}>
              <span style={{position: "absolute",top: "21px",left: "61%"}}><button type='button' style={{fontSize: "15px",background: "none",border: "none"}}  onClick={sortBy} ><FaAngleDown/></button></span>
              <span style={{position: "absolute",top: "0%",left: "61%"}}><button type='button'   style={{fontSize: "15px",background: "none",border: "none"}}    onClick={sortByDng} ><FaAngleUp/> </button></span>
            </div>
                                                            ...
                                                            <td colSpan={10}>
                                                                <table>
                                                                    <tr>
                                                                        ...
                                                                        <td style={{ width: "2%" }}></td>
                                                                        <td style={{ width: "14px" }}><PopupOfdata/></td>
                                                                        <td style={{ width: "14px", color: "red" }}><a><MdDelete /></a></td>
                                                                    </tr>
                                                                </table>
                                                            </td>
                                                            {/* <td colSpan={10}>
                                                        ...
                                                        {item.show && (
                                                            ...
                                                            <>
                                                                ...
                                                                {item.Child.map(function (childitem: any) {
                                                                     if (searchs == "" || childitem.Title.toLowerCase().includes(searchs.toLowerCase()) ) {
                                                                    return (
                                                                            ...
                                                                            {childitem.show && (
                                                                                <>
                                                                                    {childitem.Child.map(function (childinew: any) {
                                                                                         if (searchcs == "" || childinew.Title.toLowerCase().includes(searchcs.toLowerCase()) ) {
